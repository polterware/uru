...
impl ShipmentsRepository {
    ...
    pub async fn create(&self, shipment: Shipment) -> Result<Shipment> {
        ...
        sqlx::query_as::<_, Shipment>(sql)
            ...
            .bind(shipment.created_at)              // $25
            .bind(shipment.updated_at)              // $26
            .fetch_one(&self.pool)
            .await
    }
    ...
    pub async fn find_by_id(&self, id: &str) -> Result<Option<Shipment>> {
    ...
    pub async fn find_by_order_id(&self, order_id: &str) -> Result<Vec<Shipment>> {
        let sql = "SELECT * FROM shipments WHERE order_id = $1 ORDER BY created_at DESC";
