...
impl MigrationService {
    ...
    pub async fn migrate_shop(&self, shop_id: &str) -> DbResult<()> {
        ...
        self.run_migration(&pool, SHOP_SCHEMA_SQLITE, &format!("shop_{}", shop_id))
            .await
    }

    /// Run a migration script on a pool.
    async fn run_migration(&self, pool: &SqlitePool, sql: &str, name: &str) -> DbResult<()> {
        ...
        self.set_migration_version(pool, target_version).await?;
    ...
    async fn ensure_migration_table(&self, pool: &SqlitePool) -> DbResult<()> {
        sqlx::query(
            r#"
    ...
    async fn get_migration_version(&self, pool: &SqlitePool) -> DbResult<i32> {
    ...
    async fn set_migration_version(&self, pool: &SqlitePool, version: i32) -> DbResult<()> {
