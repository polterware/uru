...
export const Route = createFileRoute("/shops/$shopId/inventory/$inventoryLevelId/edit")({
  component: EditInventoryLevel,
})
...
function EditInventoryLevel() {
    ...
    const loadData = async () => {
      try {
        ...
        setFormData({
          ...
          batch_number: inventoryLevel.batch_number || "",
          serial_number: inventoryLevel.serial_number || "",
          expiry_date: inventoryLevel.expiry_date
            ? inventoryLevel.expiry_date.split("T")[0]
            : "",
          quantity_on_hand: inventoryLevel.quantity_on_hand?.toString() || "0",
          quantity_reserved: inventoryLevel.quantity_reserved?.toString() || "0",
          stock_status: inventoryLevel.stock_status || "sellable",
          aisle_bin_slot: inventoryLevel.aisle_bin_slot || "",
        })
      } catch (error) {
        console.error("Failed to load inventory level:", error)
        toast.error("Failed to load inventory level")
      } finally {
        setIsLoading(false)
      }
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h3 className="text-lg font-medium">Edit Inventory Level</h3>
        <p className="text-sm text-muted-foreground">
          Update inventory level details for the selected product and location.
        </p>
      </div>
        ...
        <div className="grid gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Update quantities for this inventory level.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
